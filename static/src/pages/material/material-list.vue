<template>
  	<div class="material-list">
  		<div v-if="userId===2" class="handle-material">
  			<button @click="removeMaterial">删除资料</button>
  		...
  		<ul>
			...
			<p class="no-news" v-if="materialTitle.length===0">暂时还没有上传资料</p>    			
  		</ul>
  	<page v-bind:page="page" v-bind:page-size="pageSize" v-bind:total="total" v-bind:max-page="maxPage" v-on:pagehandler="pageHandler" v-if="materialTitle.length!==0"></page>
  	</div>
</template>
			...
			var resource = this.$resource("http://localhost:8080/graduation/"+this.targetUrl);
			  ...
			  resource.save({nowPage:page,rows:10}).then(response => {
    			...
    			for(var i=0,len=data.length;i<len-1;i++){
    				...
    				this.materialTitle[i]["fileId"]=parseData["fileId"];
    				this.materialTitle[i]["checked"]=false;
    				this.materialTitle[i]["title"]=parseData["fileName"];
    				this.materialTitle[i]["fileURL"]="http://localhost:8080/graduation/"+parseData["fileURL"]+parseData["fileName"];
    			}
