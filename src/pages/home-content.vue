  	...
  	<div class="home-content">
  		...
  		<div class="news-list">
				<h3 class="skills-title">
					...
					<span class="title-icon" v-if="!noNews"><router-link to="/news-list">>>更多</router-link></span>
				</h3>
				<ul class="skill-article">
					...
					<p class="no-news" v-if="newsTitle.length===0">暂无任何新闻信息</p>
				</ul>
  		</div>
  	</div>
</template>
    ...
    methods:{
		getNewsTitle(page,url){
			if(this.userId&&this.userId!==undefined){
	            this.$http.get("http://localhost:8080/graduation/"+url,{nowPage:1,rows:this.rows})
	        		.then(function(res){
	        			let data=res.body[this.targetUrl];
	        			if(data&&data.length){
	        				this.noNews=false;
	        				this.newsTitle=[];
	        				data.forEach((d,i)=>{
		        				this.newsTitle.push({});
		        				this.newsTitle[i]["title"]=d["title"];
		        				this.newsTitle[i]["newsId"]=d["newsId"];
